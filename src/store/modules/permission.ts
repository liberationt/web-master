...
export const filterAsyncRoutes = (routes: any[], roles: string[]) => {
  return routes.filter((route) => {
    ...
    let prefix = ''
    if(route&&route.systemName&&route.systemName.indexOf('-') != -1){
      prefix = route.systemName.split('-')[1]
    }
    route.path = '/' + prefix + '/' + route.routeName
    // route.path = '/' + route.routeName
    if (route.parentMenuId != 0 && route.modelTypeFlag == '1' && curMenuPath == "") {
      curMenuPath = route.routeName
    }
    // if (route.modelTypeFlag === '2') {//自定义报表菜单处理逻辑
    //   debugger
...
export const kjFilterAsyncRoutes = (routes: any[], roles: string[]) => {
  return routes.filter((route) => {
    ...
    if (
      ...
      route.children = kjFilterAsyncRoutes(route.childMenuList, [])
    }
    delItem(route)
    return true
  })
