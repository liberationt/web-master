 ...
 * @LastEditors: 曹俊
 * @Description: 
 * @FilePath: \web-master\src\api\his\pha\im_herbpharmacy\inventoryEnter.ts
 */ 
...
export const initData = (data: any) => {
    return request({
        url: `/hisapi//check/entry/init/data?yfsb=${data}`,
        method: 'post',
        headers: {
            'Res-Type': 'special'
        }
...
export const queryTotal = (data: any) => {
    return request({
        url: '/hisapi/check/entry/query/total/money',
        method: 'post',
        data
    })
}
...
export const entryReset = (data: any) => {
    return request({
        url: `/hisapi/check/entry/reset?yfsb=${data}`,
        method: 'post',
    })
}
