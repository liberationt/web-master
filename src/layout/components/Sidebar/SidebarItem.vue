<template>
  ...
  >
    ...
    <template v-if="!alwaysShowRootMenu && theOnlyOneChild && !theOnlyOneChild.children">
      ...
      >
        <div @mouseenter="calcWidth">
          <el-tooltip
            effect="light"
            :content="theOnlyOneChild.meta&&theOnlyOneChild.meta.title"
            placement="bottom"
            :enterable="false"
            :manual="!isOverFlow"
          >
            ...
            >
              ...
              <span
                ...
                ref="menuText"
                class="menu-text"
              >{{ theOnlyOneChild.meta&&theOnlyOneChild.meta.title }}</span>
              <!-- 叶子菜单 -->
              <ul
                v-show="visible"
    ...
    <el-submenu v-else :index="resolvePath(item)" popper-append-to-body>
...
export default class extends Vue {
  ...
  private goto(item: any) {
    const { name } = item
    if (name) {
      TagsViewModule.addView(item)
    }
    if (item.systemName) {
      let curPath = '/' + item.systemName.split('-')[1] + '/' + item.name
      routerGo(curPath, '')
    }
  }
...
.el-menu-item.is-active {
  ...
  background-image: url('~@/assets/main-img/bgi-menu.png')!important;
