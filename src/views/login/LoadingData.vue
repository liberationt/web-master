...
import { getRegion, getPrimaryDic, getHspbasicdata, getDicJson, getYwAllDic, regionJson, getJCJYAllData, getAllRegion } from '@/api/his/basic'
import { dictExecuteSql } from '@/websql/functions'
import { updataPassWord } from '@/api/his/user/sysUser'
import { setToken } from '@/utils/cookies'
import { dicdata } from '@/resource/region.js'
...
export default class extends Vue {
  ...
  private allRegionList: Array<any> = [] //新的行政区划字典
  ...
  private init() {
    ...
    console.log(_that.tableList,'_that.tableList_that.tableList')
    _that.tableList_FB = []
    // 开始进行websql事务
    Vue.prototype.$db.transaction(function (tx: any) {
      ...
      for (let i = 0; i < _that.tableList.length; i++) {
        tx.executeSql(
          'SELECT * FROM ' + _that.tableList[i],
          [],
          function (tx: any, results: any) {
            // 查询成功
            //console.log('查到' + _that.tableList[i])
            _that.tableList_FB.push(_that.tableList[i])
            console.log(_that.tableList_FB.length,_that.tableList_FB,"_that.tableList_FB.length_that.tableList_FB.length")
            if (_that.tableList_FB.length == _that.tableList.length) {
              // 表创建结束
              _that.msg = '正在加载'
  ...
  private getRegionFun(_that: any) {
    // console.log('localStorage.regionList: ', localStorage.regionList)
    // _that.regionList = localStorage.regionList ? JSON.parse(localStorage.regionList) : []
    _that.regionList = dicdata.data.length > 0 ? dicdata.data : []
    // console.log(_that.regionList)
    // 创建行政区划表
    Vue.prototype.$db.transaction(function (tx: any) {
      _that.msg = '创建行政区划表'
      tx.executeSql(
        'CREATE TABLE IF NOT EXISTS TB_REGION (kid INT PRIMARY KEY,kname varchar(50),klevel INT,parentId INT ,wb varchar(10),py varchar(20))',
        [],
        function (tx: any, result: any) {
          ...
          _that.regionList.forEach((ele: any, i: number) => {
            tx.executeSql(
              "insert into TB_REGION (kid,kname,klevel,parentId,wb,py)values('" + ele.id + "','" + ele.name + "','" + ele.level + "','" + ele.parentId + "','" + ele.wbCode + "','" + ele.pyCode + "')",
              [],
              function (tx: any, result: any) {},
              function (tx: any, error: any) {
                //执行失败的回调函数string
                ...
                _that.msg = '行政区划主表'+ ele.name + '导入失败' + error.message
              }
            )
  ...
  // 新的行政区划表（合并）
  private async getAllRegionFun(_that: any) {
    const res = await getAllRegion()
    setTimeout(() => {
      _that.allRegionList = res.data ? res.data : []
      //console.log(_that.regionList)
      // 创建行政区划表
      Vue.prototype.$db.transaction(function (tx: any) {
        _that.msg = '创建行政区划表'
        tx.executeSql(
          'CREATE TABLE IF NOT EXISTS TB_REGION_ALL (kid INT PRIMARY KEY,kname varchar(50),klevel INT,parentId INT ,wb varchar(10),py varchar(20))',
          [],
          function (tx: any, result: any) {
            //执行成功的回调函数
            // 添加行政区划数据
            _that.msg = '行政区划表数据导入'
            _that.allRegionList.forEach((ele: any, i: number) => {
              //console.log(ele.name)
              tx.executeSql(
                "insert into TB_REGION_ALL (kid,kname,klevel,parentId,wb,py)values('" + ele.id + "','" + ele.name + "','" + ele.level + "','" + ele.parentId + "','" + ele.wbCode + "','" + ele.pyCode + "')",
                [],
                function (tx: any, result: any) {},
                function (tx: any, error: any) {
                  //执行失败的回调函数string
                  const errMsg = '行政区划表【' + ele.name + '】导入失败'
                  _that.failedList.push(errMsg)
                  _that.msg ='行政区划新主表'+  ele.name + '导入失败' + error.message
                }
              )
              if (_that.allRegionList.length - 1 == i) {
                _that.msg = '行政区划表数据导入完成'
                _that.tableList_FB.push('TB_REGION_ALL')
                if (_that.tableList_FB.length == _that.tableList.length) {
                  // 表创建结束
                  _that.msg = '正在加载'
                  _that.updPassOrLoad = false
  ...
  private async getPrimaryDicFun(_that: any) {
    ...
    _that.primaryList = res.data ? res.data : []
    // console.log(_that.primaryList)
    Vue.prototype.$db.transaction(function (tx: any) {
      _that.msg = '创建标识数据主表'
      tx.executeSql(
        'CREATE TABLE IF NOT EXISTS SYS_PRIMARY_DATA (dk varchar(100),dname varchar(20),dversion INT)',
        [],
        function (tx: any, result: any) {
          // 创建主表成功
          _that.msg = '创建标识数据子表'
          tx.executeSql(
            'CREATE TABLE IF NOT EXISTS SYS_PRIMARY_DATA_VALUE (dk varchar(100),dv varchar(20),dn varchar(50),wb varchar(10),py varchar(20),showOrder INT,stop varchar(20))',
            [],
            function (tx: any, result: any) {
              // 创建子表成功
              ...
              _that.primaryList.forEach((item: any, index: number) => {
                tx.executeSql(
                  "insert into SYS_PRIMARY_DATA (dk,dname,dversion)values('" + item.dk + "','" + item.dname + "','" + item.dversion + "')",
                  [],
                  function (tx: any, result: any) {
                  ...
                  function (tx: any, error: any) {
                    ...
                    _that.failedList.push(errMsg)
                    _that.msg = '标识数据主记录主表'+ item.dname + '导入失败' + error.message
                  }
                )
                item.son.forEach((ele: any, i: number) => {
                  tx.executeSql(
                    "insert into SYS_PRIMARY_DATA_VALUE (dk,dv,dn,wb,py,showOrder,stop)values('" + item.dk + "','" + ele.dv + "','" + ele.dn + "','" + ele.wb + "','" + ele.py + "','" + ele.showOrder + "','" + ele.stop + "')",
                    [],
                    function (tx: any, result: any) {
                      // _that.msg = '标识数据主记录表数据导入完成'
                    },
                    function (tx: any, error: any) {
                      ...
                      _that.failedList.push(errMsg)
                      _that.msg = '标识数据主记录子表'+  ele.dn + '导入失败' + error.message
                    }
                  )
                })
                if (index == _that.primaryList.length - 1) {
                  // 执行索引
                  _that.msg = '标识数据记录表数据导入完成'
                  dictExecuteSql(
                    'CREATE INDEX dk1_index ON SYS_PRIMARY_DATA (dk)',
                    function (tx: any, result: any) {
                      //console.log('CREATE INDEX dk1_index ON SYS_PRIMARY_DATA (dk) 成功执行')
                      dictExecuteSql(
                        'CREATE INDEX pid_index ON SYS_PRIMARY_DATA_VALUE (dk)',
                        function (tx: any, result: any) {
                        ...
                        function (tx: any, error: any) {
                          _that.msg = 'CREATE INDEX pid_index ON SYS_PRIMARY_DATA_VALUE执行失败'
                        }
                      )
                    },
                    function (tx: any, error: any) {
                      //console.log('*****:', error, error)
                      _that.msg = 'CREATE INDEX dk1_index ON SYS_PRIMARY_DATA 执行失败'
                    }
                  )
                }
              })
  ...
  private getDicJsonFun(_that: any) {
    getDicJson(null).then((res: any) => {
      _that.dicJsonList = res.data ? res.data : []
      // console.log(_that.dicJsonList)
      Vue.prototype.$db.transaction(function (tx: any) {
        _that.msg = '创建标识字典主表'
        tx.executeSql(
          'CREATE TABLE IF NOT EXISTS SYS_FLAG_DATA (dk varchar(100),dname varchar(20),dversion INT)',
          [],
          function (tx: any, result: any) {
            // 创建主表成功
            _that.msg = '创建标识字典子表'
            tx.executeSql(
              'CREATE TABLE IF NOT EXISTS SYS_FLAG_DATA_VALUE (dk varchar(100),dv varchar(20),dn varchar(50),wb varchar(10),py varchar(20),showOrder INT,stop varchar(20))',
              [],
              function (tx: any, result: any) {
                // 创建子表成功
                _that.msg = '标识字典数据导入'
                ...
                _that.dicJsonList.forEach((item: any, index: any) => {
                  tx.executeSql(
                    "insert into SYS_FLAG_DATA (dk,dname,dversion)values('" + item.dk + "','" + item.dname + "','" + item.dversion + "')",
                    [],
                    function (tx: any, result: any) {
                      // _that.msg = '标识数据主记录表数据导入完成'
                    },
                    function (tx: any, error: any) {
                      ...
                      _that.failedList.push(errMsg)
                      _that.msg = '标识字典主表'+item.dname + '导入失败' + error.message
                    }
                  )
                  item.son.forEach((ele: any, i: any) => {
                    tx.executeSql(
                      "insert into SYS_FLAG_DATA_VALUE (dk,dv,dn,wb,py,showOrder,stop)values('" + item.dk + "','" + ele.dv + "','" + ele.dn + "','" + ele.wb + "','" + ele.py + "','" + ele.showOrder + "','" + ele.stop + "')",
                      [],
                      function (tx: any, result: any) {
                        // _that.msg = '标识数据主记录表数据导入完成'
                      },
                      function (tx: any, error: any) {
                        ...
                        _that.failedList.push(errMsg)
                        _that.msg = '标识字典子表'+ele.dn + '导入失败' + error.message
                      }
                    )
                  })
                  if (index == _that.dicJsonList.length - 1) {
                    // 执行索引
                    _that.msg = '标识字典表数据导入完成'
                    dictExecuteSql(
                      'CREATE INDEX dk_bs_index ON SYS_FLAG_DATA (dk)',
                      function (tx: any, result: any) {},
                      function (tx: any, err: any) {
                        //console.log('索引创建失败', err.message)
                      }
                    ...
                    dictExecuteSql(
                      'CREATE INDEX pid_bs_index ON SYS_FLAG_DATA_VALUE (dk)',
                      function (tx: any, result: any) {
                        // //console.log('索引(pid) 成功执行')
  ...
  private getYwAllDicFun(_that: any) {
    getYwAllDic(null).then((res) => {
      _that.configJsonList = res.data ? res.data : []
      // console.log(_that.configJsonList)
      // return
      Vue.prototype.$db.transaction(function (tx: any) {
        _that.msg = '创建业务字典主表'
        tx.executeSql(
          'CREATE TABLE IF NOT EXISTS SYS_DICT_CONFIG (dk varchar(100),dname varchar(20),dversion INT)',
          [],
          function (tx: any, result: any) {
            // 创建主表成功
            _that.msg = '创建业务字典子表'
            tx.executeSql(
              'CREATE TABLE IF NOT EXISTS SYS_DICT_CONFIG_VALUE (dk varchar(100),dv varchar(20),dn varchar(50),ex varchar(2000),wb varchar(10),py varchar(20),showOrder INT,stop varchar(20))',
              [],
              function (tx: any, result: any) {
                // 创建子表成功
                _that.msg = '业务字典数据导入'
                ...
                _that.configJsonList.forEach((item: any, index: any) => {
                  tx.executeSql(
                    "insert into SYS_DICT_CONFIG (dk,dname,dversion)values('" + item.dk + "','" + item.dname + "','" + item.dversion + "')",
                    [],
                    function (tx: any, result: any) {
                    ...
                    function (tx: any, error: any) {
                      ...
                      _that.failedList.push(errMsg)
                      _that.msg ='业务字典主表'+ item.dname + '导入失败' + error.message
                    }
                  )
                  item.son.forEach((ele: any, i: any) => {
                    var sonStr = JSON.stringify(ele)
                    tx.executeSql(
                      "insert into SYS_DICT_CONFIG_VALUE (dk,dv,dn,ex,wb,py,showOrder,stop)values('" + item.dk + "','" + ele.dv + "','" + ele.dn + "','" + sonStr + "','" + ele.wb + "','" + ele.py + "','" + ele.showOrder + "','" + ele.stop + "')",
                      [],
                      function (tx: any, result: any) {
                      ...
                      function (tx: any, error: any) {
                        ...
                        _that.failedList.push(errMsg)
                        _that.msg = '业务字典子表'+ ele.dn + '导入失败' + error.message
                      }
                    )
                  })
                  if (index == _that.configJsonList.length - 1) {
                    // 执行索引
                    _that.msg = '业务字典表数据导入完成'
                    dictExecuteSql(
                      'CREATE INDEX dk_yw_index ON SYS_DICT_CONFIG (dk)',
                      function (tx: any, result: any) {},
                      function (tx: any, err: any) {
                        //console.log('索引创建失败', err.message)
                      }
                    ...
                    dictExecuteSql(
                      'CREATE INDEX pid_yw_index ON SYS_DICT_CONFIG_VALUE (dk)',
                      function (tx: any, result: any) {
                        // //console.log('索引(pid) 成功执行')
  ...
  private getJCJYAllDataFun(_that: any) {
    getJCJYAllData(null).then((res) => {
      _that.jcjyList = res.data ? [res.data] : []
      //console.log(_that.jcjyList)
      if (_that.jcjyList.length == 0) {
        //console.log('_that.jcjyList: ', _that.jcjyList)
        return
      }
      //console.log('不为空', _that.jcjyList)
      Vue.prototype.$db.transaction(function (tx: any) {
        _that.msg = '创建检验检查主表'
        tx.executeSql(
          'CREATE TABLE IF NOT EXISTS GY_JBBM_VERSION (dk varchar(100),dname varchar(20),dversion INT)',
          [],
          function (tx: any, result: any) {
            // 创建主表成功
            _that.msg = '创建检验检查子表'
            tx.executeSql(
              'CREATE TABLE IF NOT EXISTS GY_JBBM (JBXH INT,DMLB INT,JBMC varchar(150), JBPB varchar(30), JBBGK varchar(10), CDMZZD INT,PYDM varchar(100),JBLB INT,ICD_CODE varchar(30) ,QTXM  varchar(20) ,FJBM varchar(20),BZXX  varchar(100),XBXZ int,YXZY int,YXHZ int,YXWY int,YXSW int, YXQT int, KZFS int,ZFBZ INT)',
              [],
              function (tx: any, result: any) {
                // 创建子表成功
                _that.msg = '检验检查数据导入'
                ...
                _that.jcjyList.forEach((item: any, index: any) => {
                  tx.executeSql(
                    "insert into GY_JBBM_VERSION (dk,dname,dversion)values('" + item.dk + "','" + item.dname + "','" + item.dversion + "')",
                    [],
                    function (tx: any, result: any) {
                    ...
                    function (tx: any, error: any) {
                      ...
                      _that.failedList.push(errMsg)
                      _that.msg = '检验检查主表'+ item.dname + '导入失败' + error.message
                      //console.log(_that.msg)
                    }
                  )
                  item.son.forEach((ele: any, i: any) => {
                    tx.executeSql(
                      "insert into GY_JBBM (JBXH,DMLB,JBMC,JBPB,JBBGK,CDMZZD,PYDM,JBLB ,ICD_CODE,QTXM,FJBM,BZXX,XBXZ,YXZY,YXHZ,YXWY,YXSW,YXQT,KZFS,ZFBZ )values('" +
                        ele.jbxh +
                        "','" +
                        ele.dmlb +
                        "','" +
                      ...
                      [],
                      function (tx: any, result: any) {
                        if (i == item.son.length - 1) {
                          // 执行索引
                          _that.msg = '检验检查数据导入完成'
                          //console.log('检验检查数据导入完成')
                          dictExecuteSql(
                            'CREATE INDEX dk_jbbm_index ON GY_JBBM (JBXH)',
                            function (tx: any, result: any) {},
                            function (tx: any, err: any) {
                              //console.log('索引创建失败', err.message)
                            }
                      ...
                      function (tx: any, error: any) {
                        ...
                        _that.failedList.push(errMsg)
                        _that.msg ='检验检查子表'+  ele.jbmc + '导入失败' + error.message
                      }
                    )
                  })
                })
