<template>
  ...
  <div class="cascade-address" ref="cascadeAddress">
    <el-select v-model="provinceId" v-bind="$attrs" v-on="$listeners" filterable clearable :placeholder="$t('patientData.province')" remote :remote-method="remoteProvince" @on-query-change="provinceQueryChange" :loading="loadingProvince" @clear="provinceClear" @change="provinceChange" ref="province">
      <el-option v-for="item in provinceOps" :value="item.kid" :label="item.kname" :key="item.kid"></el-option>
    </el-select>
    <el-select v-model="cityId" remote :remote-method="remoteCity" v-bind="$attrs" v-on="$listeners" @on-query-change="cityQueryChange" filterable clearable :placeholder="$t('patientData.city')" class="nofirst" :loading="loadingCity" @clear="cityClear" @change="cityChange" ref="city">
      <el-option v-for="item in cityOps" :value="item.kid" :label="item.kname" :key="item.kid"></el-option>
    </el-select>
    <el-select v-model="areaId" remote :remote-method="remoteArea" v-bind="$attrs" v-on="$listeners" @on-query-change="areaQueryChange" filterable clearable :placeholder="$t('patientData.district')" class="nofirst" :loading="loadingArea" @clear="areaClear" @change="areaChange" ref="area">
...
export default class extends Vue {
  ...
  @Prop({ required: false, default: () => {} }) private type!: any // 新旧行政区划字典标识
  @Prop({ required: false, default: () => {} }) private manageType!: any // 新行政区划字典标识
  ...
  private init() {
    ...
    if (this.type || this.manageType) {
      const sql = 'select * from TB_REGION_ALL where klevel=' + 1 + ' and parentId=' + 0 //
      dictExecuteSql(
        sql,
        (tx: any, result: any) => {
  ...
  private provinceChange() {
    ...
    // )
    if (this.type || this.manageType) {
      const sql = 'select * from TB_REGION_ALL where klevel=' + 2 + ' and parentId=' + _that.provinceId // 市
      //+' and (wb like "%'+wb+'%" or py like "%'+py+'%" )'
      // console.log(sql)
      dictExecuteSql(
        sql,
  ...
  private cityChange() {
    ...
    // )
    if (this.type || this.manageType) {
      const sql = 'select * from TB_REGION_ALL where klevel=' + 3 + ' and parentId=' + _that.cityId // 区
      dictExecuteSql(
        sql,
        (tx: any, result: any) => {
          _that.areaChange()
  ...
  // 区清空 还原下拉项
  private areaClear() {
    this.areaOps = deepCopy(this.areaList)
  }
